...
func NewSignals(errCh <-chan error) *Signals {
	signals := &Signals{
...
func (s *Signals) Wait(logger *logrus.Entry, operator Operator) error {
	for {
		select {
		case <-s.quit:
			...
			return nil
		case err := <-s.errCh:
			logger.WithError(err).Error("Have err in group, start graceful quit")
			s.interrupt <- syscall.SIGQUIT
		case sig := <-s.interrupt:
			...
			case isSignalAvailable(sig, s.shutdown):
				s.mutex.RUnlock()
				logger.Info("Service was shutdown by system signal")
...
func removeSignal(signal os.Signal, list []os.Signal) (signals []os.Signal) {
	for ind, sig := range list {
		if sig == signal {
			signals = append(list[:ind], list[ind+1:]...) //nolint:gocritic
			return
		}
	}
	return list
